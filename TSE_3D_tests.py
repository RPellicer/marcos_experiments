...
sample_nr_echo = (
                             fe_resolution + sample_nr_dig_filt) * overSamplRatio  # number of (I,Q) TOTAL samples to acquire during a shot
...
if pe3D_step_nr < 2 :
    scale_G_pe3D_range = 0
else:
    scale_G_pe3D_range = np.linspace(-1, 1, pe3D_step_nr)
...
data = np.zeros([sample_nr_2_STOP_Seq, TR_nr, pe3D_step_nr], dtype=complex)
