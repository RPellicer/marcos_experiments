...
pess_step_nr = 1  # number of phase encoding steps in the slice direction
...
T_G_pre_fe_dur = math.ceil(
    T_G_pre_fe_dur / dt_grad) * dt_grad  # to make it integer of gradient time steps so AreaPreFE = AreaFE
...
exp = Experiment(samples=sample_nr_2_STOP_Seq,  # number of (I,Q) samples to acquire during a shot of the experiment
                 ...
                 instruction_file="TSE_2D_tests_RX_ON.txt",  # "TSE_2D_tests_RX_ON.txt",  # TSE_2D_tests.txt
                 assert_errors=False,
                 init_gpa=False)

repTimes = np.zeros(TR_nr)
...
for idxTR in range(TR_nr):
    ...
    data[:, idxTR] = exp.run()
    repTimes[idxTR] = time.time() - start_time
    print("Actual rep time: %.2f" % (repTimes[idxTR]))
    if ((repTime * 1e-3 - repTimes[idxTR]) > 0):
        time.sleep(repTime * 1e-3 - repTimes[idxTR])  # For testing purposes - Slow down time between TR

plt.figure(1001)
plt.plot(repTimes)
plt.title("Actual repetition times")
plt.xlabel("Acquisition")
plt.ylabel("Actual repetition time (s)")
